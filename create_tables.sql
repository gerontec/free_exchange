...
-- Object: lg_v_angebote
CREATE ALGORITHM=UNDEFINED DEFINER=`gh`@`localhost` SQL SECURITY DEFINER VIEW `lg_v_angebote` AS select `l`.`lagerraum_id` AS `lagerraum_id`,`l`.`anzahl_raeume` AS `anzahl_raeume`,`l`.`qm_gesamt` AS `qm_gesamt`,`l`.`preis_pro_qm` AS `preis_pro_qm`,`l`.`preis_gesamt` AS `preis_gesamt`,`l`.`beheizt` AS `beheizt`,`l`.`klimatisiert` AS `klimatisiert`,`l`.`zugang_24_7` AS `zugang_24_7`,`l`.`alarm_vorhanden` AS `alarm_vorhanden`,`l`.`rolltor` AS `rolltor`,`l`.`verfuegbar_ab` AS `verfuegbar_ab`,`l`.`bemerkung` AS `bemerkung`,`a`.`strasse` AS `strasse`,`a`.`hausnummer` AS `hausnummer`,`a`.`plz` AS `plz`,`a`.`ort` AS `ort`,`a`.`land` AS `land`,`a`.`latitude` AS `latitude`,`a`.`longitude` AS `longitude`,`u`.`email` AS `email`,`u`.`telefon` AS `telefon`,`u`.`name` AS `anbieter_name`,`l`.`erstellt_am` AS `erstellt_am`,to_days(current_timestamp()) - to_days(`l`.`erstellt_am`) AS `tage_alt`,60 - (to_days(current_timestamp()) - to_days(`l`.`erstellt_am`)) AS `tage_verbleibend` from ((`lg_lagerraeume` `l` join `lg_adressen` `a` on(`l`.`adresse_id` = `a`.`adresse_id`)) join `lg_users` `u` on(`l`.`anbieter_id` = `u`.`user_id`)) where `l`.`typ` = 'angebot' and `l`.`aktiv` = 1 and `u`.`aktiv` = 1 order by `l`.`erstellt_am` desc;
